...
import numpy as np
import pytest
from rydstate.angular import AngularKetFJ, AngularKetJJ, AngularKetKS, AngularKetLS
...
TEST_KET_PAIRS: list[tuple[AngularKetBase, AngularKetBase]] = [
    (
...
TEST_KETS: list[AngularKetBase] = [
    AngularKetLS(s_tot=1, l_r=1, j_tot=1, f_tot=1.5, species="Yb173"),
    AngularKetJJ(l_r=1, j_r=1.5, j_tot=2, f_tot=2.5, species="Yb173"),
    AngularKetFJ(f_c=2, l_r=1, j_r=1.5, f_tot=2.5, species="Yb173"),
    AngularKetLS(s_tot=0, l_r=1, j_tot=1, f_tot=1.5, species="Yb173"),
    AngularKetJJ(l_r=1, j_r=1.5, j_tot=1, f_tot=1.5, species="Yb173"),
    AngularKetKS(l_r=1, k=1.5, j_tot=1, f_tot=1.5, species="Yb173"),
]

TEST_KET_PAIRS += [(ket, ket) for ket in TEST_KETS]
...
def test_exp_q_different_coupling_schemes(ket: AngularKetBase) -> None:
    ...
    for q in all_qns:
        ...
        for scheme in coupling_schemes:
            assert np.isclose(exp_q, ket.to_state(scheme).calc_exp_qn(q))
            assert np.isclose(std_q, ket.to_state(scheme).calc_std_qn(q))
